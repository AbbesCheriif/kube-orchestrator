...
class TestIngressManager:
    def test_create_ingress(self, ing_manager: IngressManager, mock_networking_v1: MagicMock) -> None:
        ...
        ing_manager.create_ingress(
            ...
            rules=[
                {
                    "host": "example.com",
                    "http": {"paths": [{"path": "/", "pathType": "Prefix", "backend": {"service": {"name": "my-svc", "port": {"number": 80}}}}]},
                }
            ],
        )
        mock_networking_v1.create_namespaced_ingress.assert_called_once()

    def test_add_tls(self, ing_manager: IngressManager, mock_networking_v1: MagicMock) -> None:
        mock_ing = MagicMock()
        mock_ing.spec.tls = []
        ...
        ing_manager.add_tls("my-ingress", "default", hosts=["example.com"], secret_name="tls-secret")
        mock_networking_v1.patch_namespaced_ingress.assert_called_once()

    def test_delete_ingress(self, ing_manager: IngressManager, mock_networking_v1: MagicMock) -> None:
