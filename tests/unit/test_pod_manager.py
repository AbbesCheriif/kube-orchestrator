...
class TestPodManagerCreate:
    def test_create_pod_from_manifest(self, pod_manager: PodManager, mock_core_v1: MagicMock, sample_pod_manifest: dict) -> None:
        mock_core_v1.create_namespaced_pod.return_value = MagicMock(metadata=MagicMock(name="test-pod"))
        result = pod_manager.create_pod(manifest=sample_pod_manifest, namespace="default")
    ...
    def test_is_ready_true(self, pod_manager: PodManager, mock_core_v1: MagicMock) -> None:
        mock_pod = MagicMock()
        condition = MagicMock()
    ...
    def test_is_ready_false_when_no_conditions(self, pod_manager: PodManager, mock_core_v1: MagicMock) -> None:
        mock_pod = MagicMock()
