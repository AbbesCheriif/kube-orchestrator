...
class LimitRangeManager(BaseResourceManager[V1LimitRange]):
    ...
    def _build_limit_item(
        ...
        max_limit_request_ratio: dict | None = None,
    ) -> dict:
    ...
    def add_container_limits(
        ...
        default = {k: v for k, v in [("cpu", default_cpu_limit), ("memory", default_memory_limit)] if v}
        default_req = {k: v for k, v in [("cpu", default_cpu_request), ("memory", default_memory_request)] if v}
        ratio = {"cpu": max_limit_request_ratio_cpu} if max_limit_request_ratio_cpu else {}
        limits.append(self._build_limit_item(
            "Container", max_vals, min_vals,
            default or None, default_req or None, ratio or None,
        ))
    ...
    def add_pod_limits(
        ...
        max_vals = {k: v for k, v in [("cpu", max_cpu), ("memory", max_memory)] if v}
        min_vals = {k: v for k, v in [("cpu", min_cpu), ("memory", min_memory)] if v}
        limits.append(self._build_limit_item("Pod", max_vals, min_vals))
        return self.patch(name, {"spec": {"limits": limits}}, namespace)
    ...
    def add_pvc_limits(
        ...
        limits.append(self._build_limit_item("PersistentVolumeClaim", max_vals, min_vals))
        return self.patch(name, {"spec": {"limits": limits}}, namespace)
