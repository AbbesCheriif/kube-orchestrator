...
def delete(
    ...
    cascade: Annotated[bool, typer.Option("--cascade", help="Delete dependent resources")] = True,
    grace_period: Annotated[Optional[int], typer.Option("--grace-period", help="Grace period in seconds")] = None,
    force: Annotated[bool, typer.Option("--force", help="Force deletion (grace-period=0)")] = False,
    dry_run: Annotated[bool, typer.Option("--dry-run", help="Preview deletions without executing")] = False,
) -> None:
    ...
    try:
        ...
        if dry_run:
            ...
            if path.is_dir():
                deleted = deleter.delete_directory(str(path), namespace, recursive=False)
            else:
                deleted = deleter.delete_file(str(path), namespace, cascade=cascade, grace_period=effective_grace)
