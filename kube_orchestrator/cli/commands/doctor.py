...
def doctor(
    namespace: Annotated[
        str | None,
        typer.Option(
            "--namespace", "-n", help="Namespace to check (omit for cluster-wide)"
        ),
    ] = None,
    fix: Annotated[
    ...
    if fix:
        print_warning(
            "Auto-fix mode enabled — will attempt to resolve detected issues."
        )
    ...
    try:
        ...
        if score < 50:
            print_warning(
                f"Cluster health score: {score}/100 — critical issues detected."
            )
            raise typer.Exit(2)
        elif score < 80:
