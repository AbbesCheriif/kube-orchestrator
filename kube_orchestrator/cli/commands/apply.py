...
def apply(
    ...
    values: Annotated[
        Path | None,
        typer.Option("--values", "-f", help="Values file for template rendering"),
    ] = None,
    recursive: Annotated[
        bool, typer.Option("--recursive", "-r", help="Process directories recursively")
    ] = False,
    wait: Annotated[
        bool, typer.Option("--wait", help="Wait for resources to be ready")
    ] = False,
    timeout: Annotated[
        int, typer.Option("--timeout", help="Timeout in seconds when waiting")
    ] = 300,
) -> None:
    ...
    try:
        ...
        applier = ManifestApplier(
            client=client, dry_run=dry_run, force=False, server_side=False
        )

        if path.is_dir():
            results = applier.apply_directory(
                str(path), namespace, recursive=recursive, values=values_data or None
            )
        else:
            results = applier.apply_file(
                str(path), namespace, values=values_data or None
            )
