...
def deploy(
    image: Annotated[str, typer.Argument(help="Container image to deploy (e.g. nginx:latest)")],
    name: Annotated[Optional[str], typer.Option("--name", help="Deployment name (defaults to image name)")] = None,
    namespace: Annotated[str, typer.Option("--namespace", "-n", help="Target namespace")] = "default",
    replicas: Annotated[int, typer.Option("--replicas", help="Number of replicas")] = 1,
    port: Annotated[Optional[int], typer.Option("--port", help="Container port to expose")] = None,
    env: Annotated[Optional[list[str]], typer.Option("--env", "-e", help="Environment variables (KEY=VALUE)")] = None,
    cpu_request: Annotated[Optional[str], typer.Option("--cpu-request", help="CPU request (e.g. 100m)")] = None,
    memory_request: Annotated[Optional[str], typer.Option("--memory-request", help="Memory request (e.g. 128Mi)")] = None,
    ...
    print_info(f"Deploying image '{image}' as '{deployment_name}' in namespace '{namespace}'...")
    ...
    try:
        ...
        if port:
            pod_builder = pod_builder.with_ports(deployment_name, [{"containerPort": port}])
        ...
        result = manager.create_deployment(builder=builder, namespace=namespace)
        if not dry_run:
            print_success(f"Deployment '{deployment_name}' created in namespace '{namespace}' with {replicas} replica(s).")
        else:
